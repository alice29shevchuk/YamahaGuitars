...
export default function Admin() {
  ...
  const [idAddProduct,setIdAddProduct] = useState(0)
  ...
  const getUsers = () => {
   ...
   const getProducts = () =>{
    axios.get(`https://localhost:7031/api/ControllerClass/get-all-product`)
      .then( res => {
        const rest = res.data.value;
        setIdAddProduct(rest[rest.length-1].id)
  ...
  const getCategories = () => {
    axios.get(`https://localhost:7031/api/ControllerClass/get-all-category`)
      .then(res => {
        const rest = res.data.value;
        setIdAddCategory(rest[rest.length-1].id)
        ...
        <div className='container-fluid  min-vh-100'>
          <div className='row'>
            ...
            <div className='col'>
              ...
              {idCat === 1 &&<CategoriesAdmin getCategories={getCategories} idAddCategory={idAddCategory} Toggle={Toggle} dataCategories={dataCategories}/>}
              {idCat === 2 &&<ProductsAdmin IdAddProduct={idAddProduct} getProducts={getProducts} dataCategory={dataCategories} data={dataProduct}  Toggle={Toggle}/>}
              {idCat === 3 && <CustomersAdmin dataUsers={dataUsers} data={dataAdmins} Toggle={Toggle}/>}
              {idCat === 4 &&  <OrdersAdmin data={dataOrders} updateOrders={getOrders}  Toggle={Toggle}/>}
            </div>
          </div>
        </div>
      }

...
import  CardPopulars from '../Components/CardPopulars'
...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(`https://localhost:7031/api/ControllerClass/get-popular-product`)
    .then(res => {
      const rest = res.data.value;
      console.log(rest);
      this.setState({currentPagePopular: rest });
      // this.state.currentItems = this.state.items
      // this.state.currentPageNow = this.state.currentItems.slice(this.state.firstPageIndex,this.state.lastPageIndex)
    })
  }
  render() {
    return (
      <div>
          ...
          <CardPopulars onShowItem={this.onShowItem} items={this.state.currentPagePopular}></CardPopulars>
